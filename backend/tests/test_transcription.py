...
def test_time_aware_chunker():
    items = [
        ...
        {"text": "A delayed text", "start": 10.0, "duration": 2.0} # Gap of 6.5s (> 3.0s max_gap)
    ]
...
def test_process_video_webhook_native_success():
    ...
    mock_video_select.execute.return_value = MagicMock(data=[{
        "id": "test_video_uuid",
...
def test_process_video_webhook_fallback():
    ...
    with patch("youtube_transcript_api.YouTubeTranscriptApi.fetch", side_effect=Exception("No transcript")):
