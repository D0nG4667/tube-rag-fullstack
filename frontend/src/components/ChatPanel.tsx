...
export default function ChatPanel({ videoId, onSeek }: ChatPanelProps) {
  ...
  const renderMessageText = (text: string, sources: SourceChunk[] = []) => {
    const citationRegex = /\[(Transcript|Slide) @ (\d{1,2}:\d{2})\]\(cite:(transcript|slide):(\d+)\)/g
    ...
    for (const match of matches) {
      ...
      if (citeType === 'slide') {
        const found = sources.find(
          (src) => src.chunk_type === 'frame' && Math.abs(src.start_time - seconds) <= 15
        )
        if (found?.image_url) {
      ...
      parts.push(
        ...
        >
          {citeType === 'slide' ? <Eye className="w-3 h-3" /> : <Play className="w-3 h-3" />}
          <span>{type} @ {timeStr}</span>
        </button>
      )
  ...
  return (
    <div className="flex flex-col h-full border border-zinc-800/50 bg-zinc-950/40 backdrop-blur-md rounded-xl overflow-hidden relative">
      ...
      {hoveredSlideUrl && (
        ...
        >
          ...
          <img src={hoveredSlideUrl} alt="Slide Preview" className="w-full h-full object-cover" />
        </div>
      )}
      ...
      <div className="px-4 py-3 border-b border-zinc-800/50 flex items-center gap-2">
        ...
        <span className="font-semibold text-sm text-zinc-200">TubeRAG Agentic Chat</span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4">
        {messages.map((msg, idx) => (
          ...
          >
            ...
            >
              {msg.role === 'user' ? msg.text : renderMessageText(msg.text, msg.sources)}
            </div>
          </div>
        ))}
        {loading && (
          <div className="flex gap-3 self-start items-center">
            ...
            <span className="text-xs text-zinc-500 italic">TubeRAG is reasoning...</span>
          </div>
        )}
        <div ref={chatEndRef} />
      </div>
