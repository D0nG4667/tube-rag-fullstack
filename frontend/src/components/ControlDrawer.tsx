...
import { Folder, Film, Plus, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react'
...
export default function ControlDrawer({
  ...
  const handleIngest = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/ingest`, {
        method: 'POST',
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div
            ...
            className="h-full border-r border-zinc-800/50 bg-zinc-950/40 backdrop-blur-md flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="p-4 border-b border-zinc-800/50 flex items-center gap-2">
              <Folder className="w-5 h-5 text-accent-cyan" />
              <span className="font-semibold text-lg text-gradient">TubeRAG Engine</span>
            </div>

            {/* Ingestion Input Form */}
            <div className="p-4 border-b border-zinc-800/50">
              <form onSubmit={handleIngest} className="flex flex-col gap-2">
                <label htmlFor="url-input" className="text-xs text-zinc-400 font-medium">INGEST YOUTUBE VIDEO</label>
            ...
            <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
              <span className="text-xs text-zinc-400 font-medium tracking-wider">HISTORICAL VIDEO NODES</span>
              {videos.length === 0 ? (
                <div className="text-sm text-zinc-500 italic p-4 text-center">No videos ingested yet.</div>
              ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 font-medium truncate w-full">
                          ...
                          <span className="truncate">{vid.title || vid.youtube_id}</span>
                        </div>
                        <div className="flex justify-between items-center text-xs w-full">
                          <span className="text-zinc-500">ID: {vid.youtube_id}</span>
                          <span
